...
public class SimpleLinkedList<T> implements Iterable<T> {
    ...
    public SimpleLinkedListNode<T> doTask(){
        SimpleLinkedListNode<T> result = null;
        int count = 0;
        int max_count = 0;

        SimpleLinkedListNode<T> currentNode = getNode(0);
        SimpleLinkedListNode<T> polNode = null;

        while (currentNode != null){
            if ((double) currentNode.value >= 0) {
                if (count > max_count) {
                    result = polNode;
                    max_count = count;
                }
